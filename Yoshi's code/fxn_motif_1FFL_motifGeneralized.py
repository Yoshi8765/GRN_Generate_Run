...
def FFLGenerator():
    ...
    model *FFLMotif()
      ...
      var generalized_hill3 := Vm_A1*( (K_A1*p_i^H1) / (K_A1 + p_i^H1) ) + Vm_R1*( K_R1 / (K_R1 + p_i^H1) ) ;  
      
      // Reactions:
      ts1: $N => m1   ; L1 + a_m1 - d_m1*m1
      ts2: $N => m2   ; L2 + generalized_hill1 - d_m2*m2
      ts3: $N => m3   ; L3 + generalized_hill2 * generalized_hill3 - d_m3*m3
      tl1: $AA => p_i   ; a_p1*m1 - d_p1*p_i
      tl2: $AA => p2   ; a_p2*m2 - d_p2*p2
      tl3: $AA => p_o   ; a_p3*m3 - d_p3*p_o  
...
#    plt.legend([p_i, P2, p_o], ['p_i', 'P2', 'p_o'])
    ...
    #r.reset()
    print('Saving model...\n')
    r.exportToAntimony('.\motifs\FFL.txt') #export as antimony. Export directory being inside \motifs\ is important!
    return str(os.getcwd())+('\\motifs\\FFL.txt')